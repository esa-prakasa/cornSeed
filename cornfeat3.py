...
if classCode == 0:
	textPath = r"C:\Users\Esa\Pictures\_DATASET\bima20GoodAndBad\result\bad.txt"
	path = r"C:\Users\Esa\Pictures\_DATASET\bima20GoodAndBad\bad"   

if classCode == 1:
	textPath = r"C:\Users\Esa\Pictures\_DATASET\bima20GoodAndBad\result\good.txt"
	path = r"C:\Users\Esa\Pictures\_DATASET\bima20GoodAndBad\good"
...
for imgIdx in range (90,NImgFiles,1):

	img = cv2.imread(os.path.join(path,fileList[imgIdx]))
	...
	for i in range(M):
		for j in range(N):
			if edges[i,j] == 255:
				...
				#print("%d: %d  %d"%(NPt, deg, R))



	#print(" ")
	resultText ="%3.3f %3.3f "%(mean(radArr),stdev(radArr))

	radArr = [x / MaxR for x in radArr]
	resultText = str(imgIdx)+" "+resultText + "%3.3f %3.3f"%(mean(radArr),stdev(radArr))+ " "+str(classCode)+"\n"
	#print(stdev(radArr))
